...
describe('AdsList', () => {
  ...
  it('should render loading state initially', () => {
    ...
    const { container } = render(<AdsList />);
  ...
  it('should render empty state when no ads found', async () => {
    ...
    expect(await screen.findByText('Объявления не найдены', {}, { timeout: 200 })).toBeInTheDocument();
  });
  ...
  it('should handle pagination', async () => {
    mockedApiClient.getAds.mockImplementation(() => Promise.resolve({
  ...
  it('should filter ads by category', async () => {
    ...
    const categorySelect = screen.getByText(/Категория:/).parentElement?.querySelector('select') as HTMLSelectElement;
    await act(async () => {
      await user.selectOptions(categorySelect, '0');
    });
  ...
  it('should save filter preset', async () => {
    ...
    const presetNameInput = await screen.findByPlaceholderText(/Название набора/, {}, { timeout: 200 });
    await act(async () => {
  ...
  it('should handle API error gracefully', async () => {
    mockedApiClient.getAds.mockImplementationOnce(() => 
